...
class BaseRepository implements BaseRepositoryInterface
{
    ...
    // public function indexWithPaginate($perPage, $search = null, array $conditions = [], array $searchableColumns = [], array $specialSearch = [], array $customConditions = [])
    // {
    //     try {
    ...
    public function indexWithPaginate(PaginateData $formatedData)
    {
        ...
        try {
            ...
            // Apply relations dynamically
            if ($formatedData->withRelations && $formatedData->withRelations !== ['*']) {
                $query->with($formatedData->withRelations);
            }
            ...
            // Apply date conditions (e.g., created_at)
            foreach ($formatedData->filterWithDates as $column => $value) {
                if (!empty($value)) {
            ...
            if (!empty($formatedData->search) && !empty($formatedData->searchableColumns)) {
                $query->where(function ($q) use ($formatedData) {
                    ...
                    // Apply general search search in relation models (if any)
                    if (!empty($formatedData->relationModelSearch)) {
                        foreach ($formatedData->relationModelSearch as $model) {
                            // $q->orWhereHas($model['column_name'], 'LIKE', "%{$formatedData->search}%");
                            $q->orWhereHas($model['model_relation'], function ($query) use ($model, $formatedData) {
                                $query->where($model['searchable_column'], 'like', "%{$formatedData->search}%");
                            });
                        }
                    }
                });
